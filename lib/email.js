...
export async function sendPasswordResetEmail({ name, email, resetToken, resetUrl }) {
  ...
  const fullResetUrl = resetUrl || `https://philosim.app?reset_token=${resetToken}`;
...
export async function sendWelcomeEmail({ name, email }) {
  ...
  const html = `<!DOCTYPE html><html><head><style>
    ...
    <div class="content">
      ...
      <div class="cta"><a href="https://philosim.app/app">Start Your First Simulation →</a></div>
      <p>Think wisely!<br><strong>The PhiloSim Team</strong></p>
    </div>
    <div class="footer"><p>© 2026 PhiloSim by Panda Projet · Sylvain PMO Consulting</p></div>
  </div></div></body></html>`;
  return sendEmail({ to: email, subject: `Welcome to PhiloSim, ${firstName}! 🏛️`, html, text: `Welcome to PhiloSim! Start your simulation at https://philosim.app/app`, replyTo: ADMIN_EMAIL });
}
